...
public class OntoSimBL {
	...
	private static final Logger logger = LoggerFactory.getLogger(OntoSimBL.class);

	public String ontoSimBL(InputStream file, String flNm) throws Exception {
		...
		// only for classes
		Map<String, OntoSimClsModel> ontoClsMap = new HashMap<String, OntoSimClsModel>();
		
		/**
		 * 1) It saves the .owl file as InputStream in ontoFileModel
		 */
		ontoFileModel.setFileNm(flNm + OntoConst.FL_EXT);
		ontoFileModel.setFileIpStream(file);
		...
		logger.info(flNm + " : " + ontoClsMap.size());
